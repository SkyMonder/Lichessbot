...
def get_move_time(clock, board):
    ...
    if my_time < 10.0:
        print(f"[ТАЙМ-МЕНЕДЖМЕНТ] Instant Dominion Mode: время {my_time:.1f} сек, глубина 12")
        return -12   # отрицательное значение означает глубину (в секундах отрицательное не бывает, сделаем специальный флаг)
...
def get_best_move(fen, move_time, use_depth=False, depth=12):
    """Если use_depth=True, запрашиваем ход с фиксированной глубиной (для Instant Dominion)"""
    candidates = []
    timeout = (move_time + 2.0) if not use_depth else 1.0
    for url in ENGINE_URLS:
        try:
            if use_depth:
                # Отправляем запрос с параметром depth
...
def make_move(game_id, board, move_time, use_depth=False, depth=12):
    ...
    try:
        ...
        print(f"[{game_id}] >>> {move_uci} ({'глубина '+str(depth) if use_depth else str(move_time)+'s'})")
        sys.stdout.flush()
        return True
    except Exception as e:
                ...
                for event in stream:
                    if 'clock' in event:
                        ...
                        if isinstance(move_time_val, int) and move_time_val < 0:
                            use_depth = True
                            depth = -move_time_val
                            move_time_val = 0.05  # не используется, но для единообразия
                        else:
        ...
        try:
            for event in client.bots.stream_incoming_events():
                if event['type'] == 'challenge':
                    ...
                    opponent = black if my_username == white else white if my_username == black else None
                    if opponent:
                        with bully_lock:
                            if opponent in bully_data:
                                ...
                                elif info['games_left'] is not None and info['games_left'] != -1:
                                    if info['games_left'] <= 0:
                                        del bully_data[opponent]
                                        print(f"[БУЛЛИНГ] Лимит партий для {opponent} исчерпан")
                                    else:
                                        info['games_left'] -= 1
                                        ...
                                        try:
                                            ...
                                            print(f"[БУЛЛИНГ] Новый вызов {opponent}, осталось {info['games_left']}")
                                        except Exception as e:
